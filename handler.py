...
def arguments(args):
    if args.no_lrc and args.no_txt:
        ...
        base_dir = os.path.join(__get_path(), "lyrics_downloaded")
        __dir = os.path.join(base_dir, __sanitize(data.get("dir")))
        ...
        for track in data["tracks"]:
            ...
            if not args.no_lrc:
                path = os.path.join(__dir, f"{__sanitize(__file)}.lrc")
            ...
            if not args.no_txt:
                path = os.path.join(__dir, f"{__sanitize(__file)}.txt")
                if os.path.exists(path):
